...
async def pay_item(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [
        ...
        [InlineKeyboardButton("🗑️ Terminate Other Sessions", callback_data=f"term_sess_{item_id}")]
    ]
    txt = f"🎉 <b>Purchase Successful!</b>\n\n📱 <b>Phone:</b> <code>{phone}</code>\n\n<i>Enter number in Telegram app, then click GET OTP NOW below.</i>"
    await q.message.reply_text(txt, reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
...
async def fetch_live_otp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        kb = [
            [InlineKeyboardButton("📩 GET OTP AGAIN", callback_data=f"get_otp_{item_id}")],
            [InlineKeyboardButton("🗑️ Terminate Other Sessions", callback_data=f"term_sess_{item_id}")]
        ]
...
async def terminate_other_sessions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    session_str = purchase["session"]
    msg_status = await q.message.reply_text("🔄 <b>Terminating active sessions & removing bot access...</b>", parse_mode="HTML")
    ...
    try:
        ...
        if not await client.is_user_authorized():
            await msg_status.edit_text("❌ <b>Session already invalid or logged out!</b>")
            await client.disconnect()
            return
        ...
        await msg_status.edit_text(
            "✅ <b>All active sessions terminated successfully!</b>\n\n"
            "🔒 <b>Bot Access Removed:</b> I no longer have access to this Telegram account. You are now the sole owner!",
        ...
        await msg_status.edit_text(
            ...
            "<i>Note: Telegram fresh login par 24 hours tak active sessions terminate karne ki permission block kar deta hai.</i>"
        )
