...
# Render PostgreSQL Internal Database URL Yahan Paste Karein:
DATABASE_URL = os.environ.get("DATABASE_URL", "postgresql://store_db_t7gj_user:jMcIISpLCuMOXDA4N7lZ4uUJlq4KtHfA@dpg-da8ovg67bikc73csmmd0-a/store_db_t7gj)
...
def init_db():
    conn = get_db()
    c = conn.cursor()
    c.execute("""CREATE TABLE IF NOT EXISTS users (uid BIGINT PRIMARY KEY, balance REAL DEFAULT 0.0, orders INT DEFAULT 0);""")
    c.execute("""CREATE TABLE IF NOT EXISTS stock (id SERIAL PRIMARY KEY, category TEXT, age TEXT, price REAL, phone TEXT, session TEXT, is_sold INT DEFAULT 0);""")
    c.execute("""CREATE TABLE IF NOT EXISTS purchases (id SERIAL PRIMARY KEY, uid BIGINT, item_id INT, phone TEXT, session TEXT);""")
...
def get_user_db(uid):
    ...
    c.execute("SELECT balance, orders FROM users WHERE uid = %s", (uid,))
    row = c.fetchone()
    if not row:
        c.execute("INSERT INTO users (uid, balance, orders) VALUES (%s, 0.0, 0)", (uid,))
        conn.commit()
...
def update_balance_db(uid, amt):
    ...
    c = conn.cursor()
    c.execute("""
        INSERT INTO users (uid, balance, orders) VALUES (%s, %s, 0)
        ON CONFLICT(uid) DO UPDATE SET balance = users.balance + %s;
    """, (uid, amt, amt))
    conn.commit()
    c.close()
    conn.close()
...
async def category_stock_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT id, age, price FROM stock WHERE category = %s AND is_sold = 0", (cat_type,))
...
async def buy_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT id, category, age, price FROM stock WHERE id = %s AND is_sold = 0", (item_id,))
    item = c.fetchone()
    c.close()
    conn.close()
...
async def pay_item(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT id, price, phone, session FROM stock WHERE id = %s AND is_sold = 0", (item_id,))
    item = c.fetchone()
    ...
    c.execute("UPDATE users SET balance = balance - %s, orders = orders + 1 WHERE uid = %s", (price, uid))
    c.execute("UPDATE stock SET is_sold = 1 WHERE id = %s", (item_id,))
    c.execute("INSERT INTO purchases (uid, item_id, phone, session) VALUES (%s, %s, %s, %s)", (uid, item_id, phone, session_str))
    conn.commit()
    c.close()
    conn.close()
...
async def fetch_live_otp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT session, phone FROM purchases WHERE item_id = %s", (item_id,))
    purchase = c.fetchone()
    c.close()
    conn.close()
...
async def admin_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "adm_panel_add":
        ...
        await q.message.edit_text("✅ Force Join has been Turned OFF!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Admin", callback_data="btn_admin_menu")]]))
...
async def add_stock_session_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute("INSERT INTO stock (category, age, price, phone, session) VALUES (%s, %s, %s, %s, %s)",
...
def main():
    ...
    # Admin Conversation Handler for Stock
